...
import 'package:cuponex/Menu/Detalles/DetWithoutConsu.dart';
import 'package:flutter/material.dart';
...
import 'Detalles/DetalleCupon.dart'; // convertir datos del back json


const String ip ="http://10.0.2.2:3000";//10.0.2.2


class CuponesObtenidos extends StatelessWidget{
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: CuponesObtenidosful(usermode.id),

    );
  }
}

class CuponesObtenidosful extends StatefulWidget{
  CuponesObtenidosful(this.idBase);
  final String idBase;

  @override
  State<StatefulWidget> createState() {
    print(idBase);
    return new _CuponesObtenidosful(idBase);
  }
}

class _CuponesObtenidosful extends State<StatefulWidget>{
  _CuponesObtenidosful(this.id);
  final String id;
  Map data={};
  List cupDetData = [];

  getDetalleCupones(idf) async{
    http.Response response = await http.get(Uri.parse(ip+'/detCupUser/'+idf));
                ...
                children: <Widget>[
                  ...
                  if("${cupDetData[index]["_id"]}"!=null)...[
                    ...
                    GestureDetector(
                      onTap: () {
                        setState(() {
                          print("Voy a mandar el dato: "+"${cupDetData[index]["idCupon"]}");
                          Navigator.push(context, MaterialPageRoute(builder: (context){
                            return DetWitoutConsu("${cupDetData[index]["idCupon"]}");
                          }));
                        });
                        //print("Me presiono fuerte: "+"${cuponesData[index]["idCupon"]}");
                      },
                      child: Text(
                        "${cupDetData[index]["_id"]}",
                        style: const TextStyle(
                          fontWeight: FontWeight.bold,
                          fontSize: 15,
                          color: Colors.white70,
