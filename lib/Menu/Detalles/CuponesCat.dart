...
class CuponesByCatBus extends StatelessWidget{
  CuponesByCatBus(this.idBase);
  final String idBase;
  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
          title: Text('CUPONES DE LA EMPRESA'),
          backgroundColor: Colors.cyan[800],
        ),
        drawer: Drawer(
          child: ListView(
        ...
        body: CuponesByCatBusful(idBase)
    );
  }
}

class CuponesByCatBusful extends StatefulWidget{
  CuponesByCatBusful(this.idBase);
  final String idBase;
  @override
  State<StatefulWidget> createState() {
    return new _CuponesByCatBusful(idBase);
  }
}

class _CuponesByCatBusful extends State<StatefulWidget>{
  _CuponesByCatBusful(this.idBase);
  final String idBase;
  Map data={};
  List cuponesData = [];
  ...
  getEmpresasCat() async{
    // Map data;
    // List cuponesData;
    http.Response response = await http.get(Uri.parse(ip+'/cupByIdCatBuss/'+idBase));
