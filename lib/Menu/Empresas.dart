...
import 'Detalles/CuponesCat.dart'; // convertir datos del back json
...
class Empresaful extends StatefulWidget{
  @override
  State<StatefulWidget> createState() {
    return new _Empresaful();
  }
}

class _Empresaful extends State<StatefulWidget>{
  Map data={};
  List empresasData = [];
  getEmpresas() async{
    // Map data;
    // List cuponesData;
    http.Response response = await http.get(Uri.parse(ip+'/consultarEmpresas'));

    data = json.decode(response.body);
    setState(() {
      empresasData = data['data'];
    });
    print(response.body);
  }
  @override
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: ListView.builder(
          ...
          itemCount: empresasData == null ? 0 : empresasData.length,
          itemBuilder: (BuildContext context, int index){
            return Container(
              child: Row(
                children: <Widget>[
                  if("${empresasData[index]["cantidad"]}"!='0')...[
                    Icon(
                      Icons.business,
                      color: Colors.yellow ,
                      size: 50.0,
                    ...
                    GestureDetector(
                      onTap: () {
                        setState(() {
                          print("Voy a mandar el dato: "+"${empresasData[index]["_id"]}");
                          Navigator.push(context, MaterialPageRoute(builder: (context){
                            return CuponesByCatBus("${empresasData[index]["_id"]}");
                          }));
                        });
                        //print("Me presiono fuerte: "+"${cuponesData[index]["idCupon"]}");
                      },
                      child: Text(
                        "${empresasData[index]["nombre"]}",
